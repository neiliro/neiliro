...
Search runs in the browser (#216): `GET /api/search/corpus` returns the searchable rows the signed-in person may see, decrypted on the client and matched as a case-insensitive substring, three characters minimum. The trigram FTS5 index that did this on the server is kept for rows written before encryption and goes with #226; the behaviour it gave — morphology in any language without a stemmer, «переезд» finding «переезда» and «переездом» — is preserved, since a substring match over the decrypted text is exactly what a trigram index computed.
...
**The family key** (migration 033, `routes/keys.ts`, `web/src/lib/family-key.tsx`) is one random 256-bit key per family, generated in the browser of the first adult who signs in with a wrap key in hand, and stored on the server only *wrapped*: `key_envelopes` holds one envelope per door. A `password` envelope per member, opened by the wrap key sign-in derives; one `recovery` envelope for the family, opened by a code shown exactly once at creation and acknowledged on a screen that says what losing it means; and `handoff` envelopes — the key wrapped under a random secret that travels in the fragment of a link, which browsers never send — by which a member who holds the key re-admits one who does not. An invitation made on a device that holds the key carries the same construction (`invites.envelope`, migration 034, authenticated by the invite's id), so a new member holds the key from their first sign-in; the founder invitation never does, because the founder generates the key. Every password the browser did not derive a wrap key from (an administrator's reset, the e-mailed reset, `admin-reset.mjs`) retires the member's password envelope rather than leaving one that silently fails to open; a password change carries the re-wrapped envelope in the same request, so there is no moment when the password and the envelope disagree. The X25519 pair the server will seal incoming mail to is derived from the family key by HKDF, so its public half sits in `family_key` and its private half is never stored anywhere. An account that turned password sign-in off (Google only) has no wrap key at sign-in and therefore no password envelope — it is retired with the switch; such a member holds the key on their devices and admits a new one by another member's link or the recovery code, and the rule that the administrator's password cannot be disabled keeps one password-derived envelope in every family. The server cannot verify who holds the key; its proxy is a live password envelope, and that is what gates handing the key on. Phase 1 of the ADR built the doors before the first byte went through them; phase 2 sends the notes module through (#215): `web/src/lib/crypto/field-map.ts` lists which columns are sealed and why the rest are not, a test reads the migrations to insist every TEXT column of a covered table is on one list or the other, and `web/src/lib/codec.ts` seals and opens those fields inside the API client so pages never see an envelope. The browser mints the row id on create — the envelope's authenticated data names table, column and row, so the id must exist before encryption — and the server accepts it and refuses a duplicate. What the server used to derive from the text (the excerpt, the `[[link]]` rows, template expansion, title matching for links) the browser computes and sends alongside; the server stores an excerpt it cannot read and joins link rows whose `target_title` it cannot read either. A locked device — the family has a key, this browser does not — refuses to write rather than overwrite ciphertext with plaintext (`VaultLockedError`), and reads show a placeholder. Events and calendars (#218) add the one exception the ADR allows: the calendar subscription and the shared-event link carry the family key after their token (`<token>~<key>`), and `server/src/lib/envelope.ts` opens the fields those two responses need with a key that lives for the request and is never stored — a calendar app or a guest cannot decrypt, and the person who made the link chose to hand those events on; the token surface rules (unguessable, read-only, one object, one-button revoke) are unchanged, and the public prefix snapshot stays. The server also stops ordering calendars by name. Attachments (#222) use the file envelope from `web/src/lib/crypto/files.ts`: a `sealed:<id>` multipart part is stored as received under the browser's id, `attachments.encryption` (migration 036) records the kind — 0 plaintext, 1 family-key envelope, 2 sealed on ingest to the family public key — and the GET serves an encrypted file as `application/octet-stream` with an `X-Neiliro-Encryption` header, leaving the service worker's CacheFirst rule to cache ciphertext; `web/src/lib/files.ts` opens it into a blob URL, and a `PUT /api/attachments/:id` lets the job replace a plaintext file in place without changing the id note markdown points at. Profile entries (#221) — allergies and preferences, small but medical — are sealed like any other words, with birthday and role left clear for the reminder. Lists (#220) follow the calendar's pattern for their guest link and leave wishes readable on purpose — a wishlist is the family's public face. Money (#219) encrypts names, notes and places and nothing numeric; a rule-made transaction does not receive a copy of the rule's encrypted note (bound to the rule's id) but inherits it — the transactions list joins the rule's words and the codec reads them as the row's — and the server stops ordering accounts, categories and rules by name. Tasks and projects follow (#217): the server's `next_due` answer on closing an encrypted recurring task replaces the copy it used to make, since a title bound to one id cannot be copied into another — the codec creates the next occurrence and the page still receives `spawned`. Old plaintext rows pass through unchanged until the one-time job in Settings rewrites them under the key, versions included.
...
Two checks read the source instead of calling it: every `t()` key must exist in the Russian dictionary, and every migration must apply to an empty database. A third compares the demo's string tables against each other, since a string copied across rather than translated is not a missing key and nothing else would notice it. They catch what no single test can, because nothing is wrong at any one call site.
...
Under the hood the app builds a template database **per language** at startup (migrations + seeding, rebuilt daily so the seeded dates stay fresh) and clones the matching file per visitor — a copy costs milliseconds. The sample family's own content is seeded from a typed table (`lib/demo.strings.ts`), not translated after the fact: a language missing a string then fails to compile, where a post-hoc pass would leave it in English and say nothing. That is also why switching the language inside the demo hands the visitor a new sandbox — the rows are already copied, and there is nothing left to translate them with. Each request is routed to its visitor's database through an `AsyncLocalStorage` context, so route code is identical in normal and demo modes. A sandbox disappears after a couple of idle hours, on sign-out, or when the app restarts; oversized sandboxes and the least-recently-used ones past a cap are dropped too.
...
Families live in `families/<internal-id>/` under the data directory and are listed in `registry.db` next to them — the slug is registry cosmetics, renames never move files. Provisioning is one command (`node server/dist/cli/create-family.js <slug> --admin-email …`), and it ends with a **mailed invitation** rather than a bare URL: the ordinary single-use invite (migration 011) with role `admin`, issued by the service before any user exists (migration 030 lets an invite have no creator and carry the address it went to). While that invitation exists the open first-run screen is closed — a family URL that leaks opens nothing, and it is refused in the ghost's exact words, since a closed door with its own explanation would have told a stranger which subdomains are real — and the account created through it starts with its login confirmed, because receiving the link is the proof; that login is the hosted administrator's only recovery channel (`admin-reset.mjs` needs a shell they do not have), so it has to work from day one. A family provisioned with `--no-invite` keeps the open first run for the operator's own smoke tests.
...
The family's data is self-service (`routes/family.ts`, Settings → admin). **Export**: the complete archive — database via `VACUUM INTO` (WAL folded in), attachments, manifest — streamed as tar.gz in the exact layout `scripts/import.mjs` restores, so a hosted family can leave for self-hosting at any time; a round-trip test runs a settings export through the real import script. **Deletion** (hosted only): the admin proves the password, a TOTP code when enabled, and types the family's slug; the registry row flips to `deleted` (the slug is never re-issued — a stranger inheriting it would inherit bookmarks and mail), the database files and attachments are removed, and encrypted backups are left to expire on their own within 14 days. Self-hosted installs get the export but not the button: deleting the only family means erasing the instance, which belongs to the machine's owner at the filesystem. **Rename** (hosted only, once): the default slug is the operator's — `<name>-<4 random chars>`, chosen so collisions never happen — and the family gets exactly one chance to replace it, within 24 hours of first-run setup (counted from their admin's creation, not from provisioning: the link may sit in an inbox for days). One, because everything hangs off the slug — the subdomain and with it every host-only session cookie, the mail address, calendar subscription links — and moving an address a family has lived at is a cost that only grows. The registry records the spent ticket (`renamed_at`) and retires the old slug for good, for the same reason a deleted family's is never re-issued; files never move, since the folder is named by the internal id. The admin is offered the choice once at first sign-in and finds the card in Settings until the window closes; after that neither exists.
...
**Rows are isolated; the process is not, so list endpoints are capped.** Tenancy keeps one family from reading another's data, and that holds — but CPU, memory and the event loop are shared by every family on the machine, which makes an endpoint that loads a whole table a resource one family gets to spend on everybody's behalf. Measured on a throwaway machine running the release image: a family with 50k notes turned `GET /api/notes` into a 20 MB, 4.5-second response, and an unrelated family's trivial request went from 0.15 s to over 7 s while a few of those were in flight. So the collection endpoints take a `limit` and cap themselves (`/api/transactions` always did; `/api/tasks` and `/api/notes` since #183), and the ceilings that bound work per request — a calendar window of at most a year, recurrence expansion stopped at a step count, search limited and requiring three characters — exist for the same reason rather than as tidiness. The caps sit far above any real household, and the ordering means what survives them is what people actually look at. Opening a single object never goes through a list: `/api/tasks/:id` exists precisely so that following a link to one task does not fetch the family's whole task table and filter it in the browser.
