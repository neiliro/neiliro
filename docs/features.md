...
A hub with an empty database offers to create the first account right in the browser — that account becomes the administrator. Passwords are never printed to server logs. On the hosted service the first run starts in a mailbox instead: the family is created from the landing page and the founder receives a single-use invitation with the administrator role, and while that invitation is live the open first-run screen stays closed (see *Hosted* at the end).
...
A member can also **leave for good**, and the administrator can **delete** one (Settings → People → Delete user). This is not "disable": everything only that person could see is deleted — private notes with their versions and files, personal calendars and their events, personal accounts and their own movements, the profile with its birthday, allergies and preferences, sessions, and the envelopes that let the account open the family key. What they added to shared spaces stays with the family under the name *Former member*, because a household's record does not lose the grocery run when the person who wrote it moves out. Money is the careful case: a transfer between a personal account and a shared one is kept without its words, so no shared balance moves because someone left. Leaving is proven with the password and the second factor, like deleting the family; the last administrator is refused and pointed at family deletion or at handing the role on. Settings → **My data** also gives every member a readable copy of everything their account can see — one JSON file assembled in the browser, so it holds words rather than ciphertext. Both exist because the rights to a copy and to erasure belong to the person, not to the family.
...
Everything the household owns leaves in one file whenever it wants: Settings → **Family archive** streams a `tar.gz` with the database, every attachment and a manifest — the exact layout `scripts/import.mjs` restores, so a hosted family can move onto its own server (or a self-hosted one onto another machine) without asking anyone for help. Since the family key exists the words in that database are ciphertext; the key travels inside it, wrapped in every member's password envelope and in the recovery envelope, and the manifest counts those doors so the import script can say whether a password or the recovery code will open the words — or warn that nothing will. Next to it, **Download a readable copy** assembles a JSON of everything the administrator can see through the ordinary endpoints, decrypted in the browser: words on disk without a hub, files left to the archive. Together they are the GDPR portability answer, and the rule behind both is that an export never silently hands the family a copy it cannot read later.
...
With the key in place, what the family writes is encrypted in the browser before it leaves the device — notes, task and project titles, events and calendar names, list items, the words on money entries, profile entries, attachments, and letters (sealed on arrival by the server, to a public key whose private half only the family holds). Each module's section above says what stays readable to the server and why. A device that does not hold the key shows `••••••` in place of the words, opens those rows read-only, and offers the two ways back in; it never writes, because a plaintext save from there would overwrite ciphertext. Rows written before the key existed stay readable until Settings → **Family key** → *Encrypt what was written before the key* rewrites them — once for the family's shared rows, and once per member for their own private ones, since nobody else can see those.
...
On a phone the main screen starts with four quick actions: task, expense, note, list. A phone is pulled out to record something on the go — these buttons do it in one tap. The task one opens the same quick-add as `Cmd/Ctrl + K`, the expense one opens the transaction form, the note is created and opened immediately. Wide screens do not show the block: they have hotkeys and section buttons.
...
On the hosted service the family also has an **address** — the subdomain, `smiths-a1b2.neiliro.com`. The suffix is there so no two families collide, and the family gets one chance to drop it: for 24 hours after setting up the hub, the administrator can change the address once, from Settings → **Hub address** (the hub offers this at the first sign-in, too). It is one move because everything follows the address — everyone signs in again at the new one, the family mail address changes with it, old links stop working — and the old name is held back for a year rather than handed to the next family. Once the day has passed or the address has been changed, the card disappears: the address is permanent.

A family on the hosted service **creates itself** from the landing page: family name and the administrator's e-mail, nothing else. The answer on screen is the same whatever happened — the letter is the only place the new address appears, so the mailbox is the proof of ownership. The link in the letter opens the hub once and lives a week; asking again from the website sends a fresh one, and a family nobody claims is removed a day after that link could last have expired. Every family gets **30 days free without a card**, then one plan (monthly or yearly, tax included, sold and invoiced by Paddle; the checkout runs on the marketing site because the hub admits no third-party script). Settings → **Plan** shows where the family stands and holds the link to pay or manage the subscription. When the free period or the subscription ends the hub turns **read-only** — every screen still opens, everything can be read and exported, nothing can be written — for 60 days, after two letters to the administrator; then the data is removed. Families that existed before billing did are free for good.

Both documents a hosted family agrees to promise notice before a material change: when one is coming, the administrator sees a line at the top of the hub naming the day it takes effect and linking to what changes, dismissable per device, and receives the same by e-mail to the confirmed address. Members are not told — they did not sign, and the administrator is the one who can export and delete if the family disagrees.

Hosted hubs also tell search engines to stay out — every page and response carries a `noindex` header and the robots file disallows everything — so a family's address is never listed next to the sandbox demo or the marketing site.
